...
public class PlacerGameplay : MonoBehaviour
{
	[SerializeField]
	...
	private void OnSourceUse(IDragSource source)
	{
		...
		if(index < transitions.Length) play.PlayTransition(transitions[index].clip,
				transitions[index].interruptClips[Random.Range(0, transitions[index].interruptClips.Length)], fadingTime, interruptionTime);
		index++;
